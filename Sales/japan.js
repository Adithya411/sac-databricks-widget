class DatabricksSalesGenAIWidget extends HTMLElement {
  ...
  async callSalesGenAI() {
    ...
    // 🔹 Use proxy URL from attribute (HTML) or fallback for local testing
    const proxyUrl =
    ...
    try {
      ...
      console.log("Databricks response:", data);

      // 🔹 HANDLE ALL DATABRICKS PYFUNC RESPONSE FORMATS
      let answer = "No answer returned by the model.";
      ...
      if (data.predictions && Array.isArray(data.predictions)) {
        ...
        // Case 2: [["answer"]]
          ...
          typeof data.predictions[0][0] === "string"
        ) {
          answer = data.predictions[0][0];
        }
      }
...
// 🔹 Register Web Component (this MUST match widget.json tag)
